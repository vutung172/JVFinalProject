...
public class CategoryServiceImpl implements BaseService<Category>, CategoryService, DbService<Category> {
    ...
    public Category searchCategoryById(int id) {
        ...
        try {
            connection = MySQLConnect.open();
            String searchByIdQuery = "SELECT * FROM categories WHERE id = ?";
            PreparedStatement ps = connection.prepareStatement(searchByIdQuery);
            ps.setInt(1,id);
            ResultSet result = ps.executeQuery();
            while (result.next()){
                category = new Category();
                category.setId(result.getInt("id"));
                category.setName(result.getNString("name"));
                category.setDescription(result.getNString("description"));
                category.setStatus(result.getBoolean("status"));
                return category;
            }
        } catch (Exception e) {
            e.printStackTrace();
